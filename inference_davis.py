...
def main(args):
    ...
    args.output_dir = os.path.join('results', args.backbone, 'eval', args.dataset_file, inference_ckpt[:-4])
    os.makedirs(args.output_dir, exist_ok=True)
    
    args.resume = os.path.join('results', args.backbone, 'ckpt', inference_ckpt)
...
def eval_davis(results_path):
    ...
    csv_name_global = f'global_results-{argss.set}.csv'
    csv_name_per_sequence = f'per-sequence_results-{argss.set}.csv'
    ...
    print(f'Evaluating sequences for the {argss.task} task...')
    # Create dataset and evaluate
    dataset_eval = DAVISEvaluation(davis_root=argss.davis_path, task=argss.task, gt_set=argss.set)
    ...
    # Print the results
    sys.stdout.write(f"--------------------------- Global results for {argss.set} ---------------------------\n")
    print(table_g.to_string(index=False))
    sys.stdout.write(f"\n---------- Per sequence results for {argss.set} ----------\n")
...
if __name__ == '__main__':
    ...
    eval_davis(os.path.join(args.output_dir, 'valid','Davis17_annot1'))
    eval_davis(os.path.join(args.output_dir, 'valid','Davis17_annot1_full_video'))
    eval_davis(os.path.join(args.output_dir, 'valid','Davis17_annot2'))
    eval_davis(os.path.join(args.output_dir, 'valid','Davis17_annot2_full_video'))
