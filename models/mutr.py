...
from .segmentation import CrossModalFPNDecoder, VisionLanguageFusionModule, MTA, MTALayer
...
class MUTR(nn.Module):
    """ This is the MUTR module that performs referring video object detection """
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, 
        ...
        Parameters:
            ...
            num_queries: number of object queries, ie detection slot. This is the maximal number of objects
                         MUTR can detect in a video. For ytvos, we recommend 5 queries for each frame.
        ...
        mta_layer = MTALayer(d_model=hidden_dim, nhead=8, d_ffn=2048)
        self.MTA = MTA(mta_layer, 1)
    ...
    def forward(self, samples: NestedTensor, captions, targets):
        ...
        for l, (feat, pos_l) in enumerate(zip(features[-3:], pos[-3:])): 
            ...
            # vision language early-fusion
            F_tf_mask = rearrange(mask, '(b t) h w -> b (t h w)', b=b, t=t) #[b, thw]
            src_proj_l = rearrange(src_proj_l, '(b t) c h w -> (t h w) b c', b=b, t=t)
 
            text_word_features = self.MTA(tgt=text_word_features,
                                          memory=src_proj_l,
                                          memory_key_padding_mask=F_tf_mask,
                                          pos=None,
                                          query_pos=text_pos) #[length, batch_size, c]
        ...
        if self.num_feature_levels > (len(features) - 1):
            ...
            for l in range(_len_srcs, self.num_feature_levels):
                ...
                text_word_features = self.MTA(tgt=text_word_features,
                                          ...
                                          pos=None,
                                          query_pos=text_pos) #[length, batch_size, c]
                text_word_features = text_word_features.permute(1, 0, 2).contiguous() #[length, batch_size, c]->[batch_size, length, c]
        # text_sentence_features =  text_sentence_features + text_word_features[:, 0, :]
        ...
        hs_box, memory, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, inter_samples = \
                                            self.transformer(srcs, text_embed, masks, poses, query_embeds)
        # hs_box: [l, batch_size*time, num_queries_per_frame, c]
        ...
        hsl, hsbt, hsq, hsc = hs_box.shape
 
        mti_hs_box = hs_box.reshape(hsl, b, t, hsq, hsc).permute(0, 2, 1, 3, 4).contiguous()
        mti_hs = self.mti(mti_hs_box) #[l, b, q, c]
...
def build(args):
    ...
    if 'video_swin' in args.backbone:
        ...
        from .convmae import build_convmae_backbone
